    ...
    switch (tok.type) {
      case "heading": {
        if (tok.level === 1) {
          ...
          if (theme.id === "academic") {
            ...
            parts.push(`<h2 style="font-size:20px;font-weight:400;color:${c.text};margin:28px 0 0;text-align:center;">${inlineFormat(tok.content, theme, isDark)}</h2><div style="display:flex;justify-content:center;margin:8px 0 12px;"><span style="display:inline-block;width:20px;height:1.5px;background:${c.border};"></span><span style="display:inline-block;width:4px;height:4px;background:${c.accent};border-radius:50%;margin:0 4px;"></span><span style="display:inline-block;width:20px;height:1.5px;background:${c.border};"></span></div>`);
          }
        } else {
          if (theme.id === "tech") {
      ...
      case "hr": {
        ...
        if (dividerStyle === "line") {
          ...
          parts.push(`<div style="display:flex;align-items:center;padding:12px 0;margin:0 0 16px;"><div style="flex:1;height:1px;background:${c.border};"></div><span style="font-size:10px;color:${c.textTer || c.textSec};font-family:monospace;margin:0 12px;">·</span><div style="flex:1;height:1px;background:${c.border};"></div></div>`);
        } else if (theme.id === "industry") {
          parts.push(`<div style="height:2px;background:${c.primary};border-radius:1px;margin:16px 0;opacity:0.5;"></div>`);
        } else if (theme.id === "tech") {
          // WeChat rewrites <div>→<section> and strips letter-spacing/font-family.
          // Strategy: wrap with a centering <div> (WeChat keeps text-align on block
          // containers even after div→section rewrite), then use three separate
          // <span> with margin so inter-dot spacing survives without letter-spacing.
          // Avoid putting layout responsibility on a single tag so at least one
          // fallback (block centering OR span margin) always works.
          parts.push(`<div style="text-align:center;margin:12px 0 16px;line-height:1;color:${c.border};font-size:14px;"><span style="margin:0 6px;">·</span><span style="margin:0 6px;">·</span><span style="margin:0 6px;">·</span></div>`);
        } else {
          parts.push(`<div style="display:flex;justify-content:center;align-items:center;padding:16px 0;margin:0 0 16px;"><div style="width:24px;height:1px;background:${c.border};"></div><div style="width:5px;height:5px;background:${c.accent};border-radius:50%;margin:0 8px;"></div><div style="width:24px;height:1px;background:${c.border};"></div></div>`);
        }
        break;
      }
      case "image": {
      ...
      case "custom": {
        ...
        if (blockType === "media") {
          ...
          // CTA per-theme — multi-line: line1=title, line2=url/code, line3=button text
          const ctaLines = (tok.content || "").split("\n").filter(l => l.trim());
          const ctaTitle = ctaLines[0] || null;
          const ctaUrl = ctaLines[1] || null;
          const ctaBtn = ctaLines[2] || null;
          if (theme.id === "story") {
            parts.push(`<div style="text-align:center;padding:20px 0;margin:0 0 16px;">${ctaTitle ? `<p style="font-size:14px;color:${c.textSec};margin-bottom:16px;">${ctaTitle}</p>` : ""}<div style="display:inline-block;padding:10px 32px;border:1px solid ${c.primary};color:${c.primary};font-size:13px;border-radius:2px;letter-spacing:2px;">${ctaBtn || elementConfig.ctaDefaultText}</div></div>`);
          } else if (theme.id === "industry") {
            parts.push(`<div style="background:${c.primary};border-radius:2px;padding:14px 20px;display:flex;justify-content:space-between;align-items:center;margin:0 0 16px;"><div><div style="font-size:14px;color:#fff;font-weight:600;">${ctaTitle || "订阅更新"}</div>${ctaUrl ? `<div style="font-size:11px;color:rgba(255,255,255,0.7);margin-top:4px;">${ctaUrl}</div>` : ""}</div><div style="padding:6px 18px;background:#fff;color:${c.primary};font-size:12px;border-radius:2px;font-weight:600;flex-shrink:0;">${ctaBtn || elementConfig.ctaDefaultText}</div></div>`);
          } else if (theme.id === "tech") {
            parts.push(`<div style="border:1px solid ${c.border};border-radius:2px;padding:14px 18px;margin:0 0 16px;">${ctaTitle ? `<div style="font-family:Consolas,'Courier New','Liberation Mono',monospace;font-size:13px;font-weight:600;color:${c.primary};margin-bottom:10px;letter-spacing:0.5px;">${ctaTitle}</div>` : ""}${ctaUrl ? `<div style="background:${isDark ? c.surface : lc.bg};padding:8px 14px;border-radius:2px;font-family:Consolas,'Courier New','Liberation Mono',monospace;font-size:12px;color:${c.primary};word-break:break-all;">${ctaUrl}</div>` : ""}<div style="margin-top:${ctaTitle || ctaUrl ? '12px' : '0'};display:flex;">${ctaBtn ? `<span style="padding:5px 14px;background:${c.primary};color:#fff;font-size:12px;border-radius:2px;">⭐ ${ctaBtn}</span>` : ""}<span style="padding:5px 14px;border:1px solid ${c.primary};color:${c.primary};font-size:12px;border-radius:2px;${ctaBtn ? 'margin-left:10px;' : ''}">${elementConfig.ctaDefaultText}</span></div></div>`);
          } else {
            // academic
            parts.push(`<div style="text-align:center;padding:16px 0;margin:0 0 16px;">${ctaTitle ? `<div style="font-size:12px;color:${c.textSec};margin-bottom:12px;">${ctaTitle}</div>` : ""}<div style="display:inline-block;padding:8px 28px;background:${c.primary};color:#fff;font-size:13px;border-radius:2px;">${ctaBtn || elementConfig.ctaDefaultText}</div></div>`);
          }
          ...
          const qrWrapStyle = `width:48px;height:48px;background:${isDark ? c.surface : lc.bg};border:1px solid ${c.border};border-radius:2px;overflow:hidden;`;
          const qrFallbackStyle = `width:100%;height:100%;display:flex;align-items:center;justify-content:center;font-size:9px;color:${c.textTer || c.textSec};text-align:center;`;
          const qrHtml = qrImageUrl
            ? `<div style="${qrWrapStyle}"><img src="${qrImageUrl}" alt="${elementConfig.qrLabel}" style="display:block;width:100%;height:100%;object-fit:cover;" onerror="this.style.display='none';this.nextElementSibling.style.display='flex';" /><span style="${qrFallbackStyle}display:none;">${elementConfig.qrLabel}</span></div>`
            : `<div style="${qrWrapStyle}"><span style="${qrFallbackStyle}">${elementConfig.qrLabel}</span></div>`;
          parts.push(`<div style="border-top:1px solid ${c.border};padding-top:16px;margin:16px 0 0;"><div style="display:flex;justify-content:space-between;align-items:flex-start;"><div><div style="font-size:13px;font-weight:600;color:${c.text};">${brandName}</div><div style="font-size:11px;color:${c.textSec};margin-top:4px;">${tagline}</div></div>${qrHtml}</div>${tok.content ? `<div style="margin-top:12px;font-size:11px;color:${c.textSec};">${inlineFormat(tok.content, theme, isDark)}</div>` : ""}<div style="margin-top:10px;font-size:10px;color:${c.textTer || c.textSec};">${copyrightText}</div></div>`);
