...
4. **记忆极简原则** — 依赖 Claude Code 原生 memory，agent-memory/ 只存储模型难以推导的用户偏好
...
| Agent | Model | 阶段 | 职责 |
|-------|-------|------|------|
| orchestrator | sonnet | 全流程 | 编排 pipeline、调度子 agent、用户交互 |
...
Skill 按微信公众号标准工作流阶段组织，名称对应工作流步骤。

**领域 Skill（wechat-article）**
...
| `creation-reviewing` | 复盘 | 创作复盘（diff + 记忆更新） |
...
| Skill | 说明 |
|-------|------|
| `pipeline-orchestrating` | 编排器（orchestrator agent 专用） |
| `style-profiling` | 七维度风格 DNA 提取 |
| `format-linting` | 确定性 Markdown 格式校验 |
