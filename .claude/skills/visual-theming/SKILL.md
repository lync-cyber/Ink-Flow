...
- 每张 SVG 最多使用 **3 种颜色**（从栏目色板选取），避免视觉噪音
- 禁止纯黑 `#000000`、纯白 `#ffffff`、栏目色板之外的任意 hex 值
- 不同层级用同一色系的不同明度（如 `background` 浅色填充 + `primary` 深色描边 + `text` 颜色文字）
- 颜色区分含义时必须搭配第二视觉线索（虚线/不同形状/不同填充），不可仅靠颜色区分
...
| 说明图 | primary（主轮廓）| accent（内部结构）| 标注用 textSecondary |
| 结构图嵌套 | background（外层填充）| backgroundDeep（中层填充）| 描边用 primary |
...
根据**用户意图动词**选择图表类型，而非默认用流程图：

| 用户意图/问题类型 | 图表类型 | 优先格式 | 典型触发语 |
|------------------|---------|---------|-----------|
| 步骤/流程/怎么做 | 流程图 | Mermaid | "X 的流程"、"分几步" |
| 架构/组成/在哪里 | 结构图 | SVG 嵌套层 | "X 由什么组成"、"X 的架构" |
| 原理/机制/怎么工作 | 说明图 | SVG 截面/空间隐喻 | "X 怎么工作"、"X 的原理" |
| 对比/区别/选哪个 | 对比表/双栏 SVG | 视复杂度 | "X vs Y"、"X 和 Y 的区别" |

**同一主题不同动词 → 不同图表**:
- "LLM 训练步骤" → 流程图 | "LLM 怎么工作" → 说明图 | "transformer 架构" → 结构图

**多图策略**: 当需要展示 6+ 组件时，不要一张图画全。拆分为：主干概览图（高层抽象，无 N:N 箭头网）+ 子流程细节图（每张 3-4 节点）。多张 SVG 之间必须有文字段落间隔，禁止背靠背堆叠。
...
| 说明图 | SVG | 需要空间隐喻或截面展示内部机制 | 用户要参考文档而非直觉理解 |
