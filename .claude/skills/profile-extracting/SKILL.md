...
description: >
  从目标描述或样本文章提取结构化 Profile 插件包（principles / voice / typesetting / constraints）。
  两种触发：
  （1）goal：输入栏目名 / 写作目标描述 → 推导四 slot 初始配置；
  （2）sample：输入 1-5 篇参考文章（本地路径 / content/references/ / 微信公众号 URL）→ 从样本反推风格。
  触发词："提取 profile"、"生成 profile"、"学这几篇"、"从零起一套风格"、"分析风格"、"提取风格 DNA"、"对标"。
argument-hint: "[goal | sample] [--id=<new-profile-id>] [资料路径...]"
allowed-tools: Read, Write, Edit, Glob, Grep, WebFetch, Agent, AskUserQuestion, Bash
---

# Profile Extractor

统一入口，两种模式。按模式 Read 对应 reference 执行；本主文件不展开流程细节。
...
两种模式最终都产出同一目录结构（详见 `framework/contracts/profile-protocol.md § 2.1`）：
...
- **URL 抓取失败不阻塞**：sample 模式 URL 抓取失败时优雅降级到"粘贴正文"交互
...
- `.claude/skills/profile-extracting/scripts/wechat.py` — 微信 URL 抓取（sample 模式 Step 2a 用）
