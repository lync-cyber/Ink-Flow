...
- **图表类型选择**: 根据意图动词选型（见 visual-theming skill「图表类型决策树」），不要默认全用流程图
- **禁止背靠背视觉断点**: 两个连续视觉断点之间必须有至少一个文字段落；6+ 组件拆分为概览图 + 细节图，分置不同 section
...
- **research 跳过时的降级行为**: 若 research.md 不存在（`brief.skip_research == true`），仅依据 brief.md 构建大纲；无时效标记可继承时，对所有涉及具体数字/版本/价格的关键细节主动标注 `[发布前刷新]`；不确定项段留空并注明"调研已跳过"
## Format
