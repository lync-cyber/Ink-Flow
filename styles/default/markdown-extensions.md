...
标准 Markdown 图片语法 `![图注](url)` 即可，typesetter 自动渲染为居中展示 + 图注。

- 建议宽度 ≤ 640px（platform-base rule），使用透明底 PNG 避免深色模式白边
- GIF 动图使用相同语法，自动播放
- story 栏目图注右对齐+斜体，其他栏目居中 + 11px 辅助色

| 栏目 | 常见图片类型 |
|------|------------|
| 学术前沿 | 论文中的图表截图、实验结果可视化 |
| 行业趋势 | 产品截图、现场照片、数据报告截图 |
| 技术专题 | 代码运行截图、UI 效果对比、操作 GIF |
| 人物故事 | 人物照片、工作场景、相关项目截图 |
...
紧凑排版的文末引用区，12px 字号、accent 色编号、浅色背景。替代普通有序列表，提升移动端阅读体验。引用超过 5 条时自动启用滑动窗口（max-height: 280px），避免长列表占据过多屏幕空间。
